...
class MembranesCleaningPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text="Membranes – Cleaning Page", font=("Arial", 14)).grid(row=0, column=0, columnspan=2, pady=20)
        ...
        #Current CWF
        tk.Label(self, text="Enter current CWF:", font=("Arial", 12)).grid(row=3, column=0, sticky="w", padx=5, pady=5)
    ...
    def save_values(self):
        ...
        #Current CWF
        try:
            value = float(self.curr_cwf_var.get())
            self.controller.curr_cwf = value
        except ValueError:
            messagebox.showerror("Invalid input for 'Current CWF'", "Please enter a number.")
            return
